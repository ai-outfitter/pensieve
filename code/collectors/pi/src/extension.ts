 ...
 */
import { PensieveClient } from "@pensieve/collector-core";
import { buildContext, clientOptions } from "@pensieve/collector-core";
import { CommitWatcher } from "@pensieve/collector-core";
...
export default function pensieveCollector(pi: ExtensionAPI): void {
	...
	const watcher = new CommitWatcher(context, client);

	pi.on("session_start", async () => {
		await watcher.start();
		const record = watcher.current().base("session");
		const result = await client.submit({ ...record, argv: process.argv.slice(2) });
		watcher.current().note("session", result.digest);
	});

	// The model request payload. No other harness of the three exposes this.
	pi.on("before_provider_request", async (event) => {
		const payload = (event as { payload?: unknown }).payload;
		const record = watcher.current().base("model-exchange");
		const result = await client.submit({ ...record, direction: "request", payload });
		watcher.current().note("model-exchange", result.digest);
	});
	...
	pi.on("tool_result", async (event) => {
		...
		const record = watcher.current().base("tool-call");
		...
		watcher.current().note("tool-call", stored.digest);
